...
class content_controller extends Controller
{
    ...
    public function getContentBaseUserFollow(Request $req)
    {
        ...
        u.nama as namaUser,
        (select count(*) from comment cm where cm.id_user = u.id) as jmlComment
        from content c join user ur on c.id_responder = ur.id join user u on c.id_user = u.id  where c.id_responder in (select id_responder from folow f where f.id_user={$req->id_user}) or c.id_user = {$req->id_user} order by c.create_at desc");
